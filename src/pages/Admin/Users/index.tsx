...
export default function Users() {
  ...
  const handleDeleteUser =
    (userId: any) => (event: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <Space size="middle">
          <span onClick={handleEditUser(record.id)} className="admin-edit-btn">
            Edit
          </span>
          <span
  ...
  return (
    <>
      ...
      <Table
        columns={columns}
        key={adminUsers.users.length}
        dataSource={adminUsers.users.map((user: any, idx: number) => ({
          ...user,
          key: user.id + " " + idx,
        }))}
      />
      ...
      >
        ...
        <br /> <br />
        <Checkbox
          checked={currentUserAdmin}
          onChange={(e) => setCurrentUserAdmin(e.target.checked)}
        >
          Is Admin
        </Checkbox>
      </Modal>
